...
- **Global scope**: `C:/Repo/blog_sertao/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Repo/blog_sertao/.claude/notes/`)
...
2. Glob `C:/Repo/blog_sertao/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Repo/blog_sertao/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Repo/blog_sertao/.claude/notes/` — works in any directory
