...
@C:/Repo/blog_sertao/.claude/get-shit-done/references/thinking-models-research.md
...
```bash
node "C:/Repo/blog_sertao/.claude/get-shit-done/bin/gsd-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
...
| **Live service config** | What external services have this string in their configuration — but that configuration lives in a UI or database, NOT in git? | n8n workflows not exported to git (only exported ones are in git), Datadog service names/dashboards/tags, Tailscale ACL tags, Github Tunnel names |
