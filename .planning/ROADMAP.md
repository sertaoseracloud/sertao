...
**Goal:** Repositório Astro escaffoldado, paleta semântica configurada, deploy `hello world` em `sertaoseracloud.com` via Cloudflare Pages funcionando.

**Success criteria:**
- [ ] `pnpm dev` roda localmente em <5s
- [ ] `pnpm build` produz `dist/` estático sem warnings
- [ ] `sertaoseracloud.com` retorna HTTP 200 com SSL válido via Cloudflare Pages
...
- [ ] `package.json` scripts: `dev`, `build`, `preview`, `sync:devto` (stub)

**Scope in:**
- Astro 6 scaffold via `pnpm create astro@latest -- --template blog --typescript strict`
- Tailwind v4 via Vite plugin
- Cloudflare Pages conexão com repo
- Domínio custom + SSL
...
**Dependencies:**
- Blocker autoral: conta Cloudflare Pages + DNS de `sertaoseracloud.com`
...
**Success criteria:**
- [ ] Script `scripts/sync-devto.ts` (Node 22 + `fetch` nativo + `@anthropic-ai/sdk`)
- [ ] DevToClient: `GET /api/articles?username=sertaoseracloud` + per-article fetch de `body_markdown`
...
- [ ] Cloudflare Web Analytics snippet em `BaseLayout` (sem cookie banner necessário)
...
- [ ] Cloudflare Pages buildou e deployou
...
- `docs/deploy-troubleshooting.md` (Phase 1) — gotchas do Cloudflare Pages
